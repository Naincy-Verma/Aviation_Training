...
<div class="p-8 bg-white">
    ...
    <form action="{{ route('about.update', $about->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $about->title) }}" class="w-1/2 border rounded-lg p-2" required>
        </div>

        <!-- Subtitle -->
        <div>
            <label class="block text-gray-700 font-bold mb-1">Subtitle</label>
            <input type="text" name="sub_title" value="{{ old('sub_title', $about->sub_title) }}" class="w-1/2 border rounded-lg p-2">
        </div>

        <!-- Description 1 -->
        <div>
            <label class="block text-gray-700 font-bold mb-1">Description 1</label>
            <textarea name="description_1" rows="4" class="w-1/2 border rounded-lg p-2" required>{{ old('description_1', $about->description_1) }}</textarea>
        </div>

        <!-- Description 2 -->
        <div>
            <label class="block text-gray-700 font-bold mb-1">Description 2</label>
            <textarea name="description_2" rows="4" class="w-1/2 border rounded-lg p-2">{{ old('description_2', $about->description_2) }}</textarea>
        </div>
